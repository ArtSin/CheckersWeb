...
(function (PieceType) {
    ...
    PieceType[PieceType["King"] = 2] = "King";
})(PieceType || (PieceType = {}));
;
// Цвет игрока
var PlayerColor;
...
class Board {
    ...
    static GetPositionColor(row, col) {
        return ((row + col) % 2) != 0;
    }
    // Совершение хода
    DoMove(move) {
    ...
    GetMoves(player, cell, canCapture) {
        let moves = [];
    ...
    GetMovesDFS(currMove, moves, canCapture) {
        // Начальное поле
    ...
    CanCapture(r, c, player, isKing, used) {
        // Обычная шашка
        if (!isKing) {
            // Ход по диагонали на 1 поле
            for (let dR of [-1, 1]) {
