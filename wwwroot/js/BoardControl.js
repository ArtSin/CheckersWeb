...
class BoardControl {
    constructor() {
        ...
        this.availableMoves = [];
        // Доступные поля
        this.availableCells = new Array(Board.SIZE);
        // Количество использованных полей за ход
        this.moveIndex = 0;
        // Использованные поля
        this.usedCells = new Array(Board.SIZE);
        // Последний ход
        this.lastMove = null;
        this.canvas = null;
        this.ctx = null;
    ...
    Reset(invalidate = false, resetLastMove = false) {
    ...
    OnClick(e, boundingClientRect) {
        if (gameState != GameState.Running || thisPlayer != currPlayer)
            return;
        // Ход текущего игрока
        // Координаты поля по координатам мыши
        let [row, col] = BoardControl.GetCellCoords([e.clientX - boundingClientRect.left, e.clientY - boundingClientRect.top], [this.canvas.clientWidth, this.canvas.clientHeight]);
        ...
        // Иначе выбрать поле для начала хода
            ...
            // Доступные ходы
            this.availableMoves = board.GetMoves(thisPlayer, this.selectedCell, board.CanPlayerCapture(thisPlayer));
        ...
        // Ход уже выполняется
            ...
            var newAvailableMoves = this.availableMoves.filter((move) => move.Cells[this.moveIndex + 1].Row == row && move.Cells[this.moveIndex + 1].Col == col);
    ...
    // Координаты поля по координатам мыши
    static GetCellCoords(mouseCoords, boardSize) {
        // Размер поля
    ...
    Paint() {
        ...
        if (this.lastMove != null) {
            for (let i = 0; i < this.lastMove.Cells.length - 1; i++) {
                ...
                this.ctx.moveTo((currCell.Col + 0.5) * cellSize, (Board.SIZE - 0.5 - currCell.Row) * cellSize - 15);
                this.ctx.lineTo((nextCell.Col + 0.5) * cellSize, (Board.SIZE - 0.5 - nextCell.Row) * cellSize - 15);
                this.ctx.stroke();
            }
            for (let pr of this.lastMove.Used) {
                this.ctx.save();
                this.ctx.translate((pr.Col + 0.5) * cellSize, (Board.SIZE - 0.5 - pr.Row) * cellSize - 15);
    ...
    // Отрисовка шашки
    PaintPiece(ctx, cellSize, color1, color2) {
        ctx.moveTo(0, 0);
...
BoardControl.WHITE_CELL_COLOR = "rgb(255, 206, 158)";
BoardControl.BLACK_CELL_COLOR = "rgb(209, 139, 71)";
// Цвета для выделенных, доступных, использованных полей
BoardControl.SELECTED_CELL_COLOR = "rgb(255, 0, 0)";
BoardControl.AVAILABLE_CELL_COLOR = "rgb(0, 128, 0)";
BoardControl.USED_CELL_COLOR = "rgb(0, 0, 255)";
BoardControl.MAIN_LINE_WIDTH = 3.0;
// Цвета для последнего хода
BoardControl.LAST_MOVE_COLOR = "rgb(128, 128, 128)";
BoardControl.LAST_MOVE_USED_COLOR = "rgb(148, 0, 211)";
BoardControl.LAST_MOVE_LINE_WIDTH = 5.0;
